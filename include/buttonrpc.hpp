...
class buttonrpc {
 public:
  ...
  class value_t {
   public:
    typedef typename type_xx<T>::type
        type;  //通过typename告诉编译器type_xx<T>::type 是一个类型
 ...
 private:
  ...
  template <typename F, typename S>
  void callproxy(F fun, S* s, Serializer* pr, const char* data,
                 int len);  //类成员函数
  ...
  // PROXY CLASS
  // MEMBER，function不能包装类成员变量或函数，需要配合Bind,传入函数地址和类对象地址
...
Serializer* buttonrpc::call_(std::string name, const char* data, int len) {
  ...
  if (m_handlers.find(name) == m_handlers.end()) {  //如果没有找到函数
    ...
    (*ds) << value_t<int>::msg_type("function not bind: " +
                                    name);  //设置错误信息
    return ds;
  }
...
template <typename F>
void buttonrpc::callproxy(F fun, Serializer* pr, const char* data,
                          int len)  //代理普通函数
{
