...
*/
template <typename Key, typename Value> class SkipListNode {
public:
  Key key;
  Value value;
...
public:
  ...
  std::shared_ptr<SkipListNode<Key, Value>>
  searchItem(const Key &key);           //查找节点
...
template <typename Key, typename Value>
std::shared_ptr<SkipListNode<Key, Value>>
SkipList<Key, Value>::searchItem(const Key &key) {
  mutex.lock();
...
//打印跳表
template <typename Key, typename Value> void SkipList<Key, Value>::printList() {
  mutex.lock();
...
template <typename Key, typename Value> int SkipList<Key, Value>::size() {
  mutex.lock();
