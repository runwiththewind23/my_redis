 ...
 * @Date: 2024-08-22 12:04:21
 * @LastEditors: runwiththewind23 19945243142@163.com
 * @LastEditTime: 2024-08-22 13:13:02
 * @FilePath: /My_Redis/include/Serializer.hpp
...
/*
    ...
    并且可以动态地增加或减少元素。这使得 std::vector<char>
   非常适合用作字节流的缓冲区
...
class StreamBuffer : public vector<char> {
 public:
  ...
  const char* current() { return &(*this)[m_curpos]; }  // 获取当前位置的数据
...
inline void Serializer::output_type(std::string& in) {
  ...
  in.insert(in.begin(), m_iodevice.current(),
            m_iodevice.current() + len);  //输入到in中
  m_iodevice.offset(len);
}
