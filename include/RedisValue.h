...
class RedisValue {
 public:
  ...
  template <class V,
            typename std::enable_if<
                std::is_constructible<
                    RedisValue, decltype(*std::declval<V>().begin())>::value,
                int>::type = 0>
  ...
  static std::vector<RedisValue> parseMulti(
      const std::string& in, std::string::size_type& parserStopPos,
      std::string& err);

  static std::vector<RedisValue> parseMulti(const std::string& in,
                                            std::string& err);
