...
void RedisServer::printStartMessage() {
  ...
  std::string initMessage =
      "[PID] DATE * The server is now ready to accept connections on port PORT";
    ...
    if (!tokens.empty()) {
      ...
      if (command == "quit" || command == "exit") {
        ...
        std::shared_ptr<CommandParser> commandParser =
            flyweightFactory->getParser(command);  //获取解析器

        try {
          responseMessage = commandParser->parse(tokens);
        } catch (const std::exception& e) {
          responseMessage =
              "Error processing command '" + command + "': " + e.what();
        }
        responseMessagesList.emplace_back(responseMessage);
      }
    }
  ...
  if (bytesRead > 0) {
    ...
    if (!tokens.empty()) {
      ...
      if (command == "quit" || command == "exit") {
        ...
        if (!startMulti) {
          std::shared_ptr<CommandParser> commandParser =
              flyweightFactory->getParser(command);
...
//替换字符串中的指定字符
void RedisServer::replaceText(std::string& text,
                              const std::string& toReplaceText,
