...
void RedisHelper::flush() {
  ...
  while (currentNode != nullptr) {
    ...
    if (!key.empty()) outputFile << key << ":" << value.dump() << std::endl;
...
std::string RedisHelper::select(int index) {
  ...
  std::string filePath =
      getFilePath();  //根据选择的数据库，修改文件路径，然后加载
...
// OK
std::string RedisHelper::rename(const std::string& oldName,
...
std::string RedisHelper::set(const std::string& key, const RedisValue& value,
                             const SET_MODEL model) {
  if (model == XX) {
...
std::string RedisHelper::setnx(const std::string& key,
...
std::string RedisHelper::setex(const std::string& key,
                               const RedisValue& value) {
  auto currentNode = redisDataBase->searchItem(key);
  if (currentNode == nullptr) {
...
std::string RedisHelper::incr(const std::string& key) { return incrby(key, 1); }
...
std::string RedisHelper::append(const std::string& key,
...
RedisHelper::RedisHelper() {
  FileCreator::createFolderAndFiles(DEFAULT_DB_FOLDER, DATABASE_FILE_NAME,
...
std::string RedisHelper::lpush(const std::string& key,
                               const std::string& value) {
  auto currentNode = redisDataBase->searchItem(key);
  std::string resMessage = "";
  int size = 0;
  if (currentNode == nullptr) {
    ...
    if (currentNode->value.type() != RedisValue::ARRAY) {
      resMessage = "The key:" + key + " " +
                   "already exists and the value is not a list!";
      return resMessage;
    } else {
...
std::string RedisHelper::rpush(const std::string& key,
                               const std::string& value) {
  auto currentNode = redisDataBase->searchItem(key);
  std::string resMessage = "";
  int size = 0;
...
std::string RedisHelper::lrange(const std::string& key,
...
std::string RedisHelper::hset(const std::string& key,
...
std::string RedisHelper::hdel(const std::string& key,
                              const std::vector<std::string>& filed) {
  auto currentNode = redisDataBase->searchItem(key);
  std::string resMessage = "";
  int count = 0;
...
std::string RedisHelper::hvals(const std::string& key) {
  ...
  if (currentNode == nullptr) {
    ...
    if (currentNode->value.type() != RedisValue::OBJECT) {
      ...
      for (auto hkey : valueMap) {
        resMessage +=
            std::to_string(index) + ") " + hkey.second.stringValue() + "\n";
        index++;
      }
      resMessage.erase(resMessage.size() - 1);
    }
