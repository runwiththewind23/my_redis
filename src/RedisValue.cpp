...
RedisValue::RedisValue(const std::string& value)
    : redisValue(std::make_shared<RedisString>(value)) {}

RedisValue::RedisValue(std::string&& value)
    : redisValue(std::make_shared<RedisString>(std::move(value))) {}

RedisValue::RedisValue(const char* value)
    : redisValue(std::make_shared<RedisString>(value)) {}

RedisValue::RedisValue(const RedisValue::array& value)
    : redisValue(std::make_shared<RedisList>(value)) {}

RedisValue::RedisValue(RedisValue::array&& value)
    : redisValue(std::make_shared<RedisList>(std::move(value))) {}

RedisValue::RedisValue(const RedisValue::object& value)
    : redisValue(std::make_shared<RedisObject>(value)) {}

RedisValue::RedisValue(RedisValue::object&& value)
    : redisValue(std::make_shared<RedisObject>(std::move(value))) {}

/************* Member Functions ******************/

RedisValue::Type RedisValue::type() const { return redisValue->type(); }

std::string& RedisValue::stringValue() { return redisValue->stringValue(); }
...
RedisValue& RedisValue::operator[](size_t i) { return (*redisValue)[i]; }

RedisValue& RedisValue::operator[](const std::string& key) {
  return (*redisValue)[key];
}

std::string& RedisValueType::stringValue() { return statics().emptyString; }
...
RedisValue& RedisValueType::operator[](size_t) { return staticNull(); }
...
bool RedisValue::operator==(const RedisValue& other) const {
  if (redisValue == other.redisValue) return true;
  if (redisValue->type() != other.redisValue->type()) return false;
  return redisValue->equals(other.redisValue.get());
}

bool RedisValue::operator<(const RedisValue& other) const {
  if (redisValue == other.redisValue) return false;
...
void RedisValue::dump(std::string& out) const {
  redisValue->dump(
      out);  // 调用JsonImpl类的dump函数将Json对象转化为JSON字符串并追加到out中
}
...
RedisValue RedisValue::parse(const std::string& in, std::string& err) {
  ...
  if (parser.failed) return RedisValue();  // 如果解析失败，返回一个空的Json对象
  if (parser.i != in.size())
    return parser.fail(
        "unexpected trailing " +
        esc(in[parser.i]));  // 如果输入字符串尚有未解析内容，报告错误
...
std::vector<RedisValue> RedisValue::parseMulti(
  ...
  while (parser.i != in.size() && !parser.failed) {
    jsonList.push_back(
        parser.parseRedisValue(0));  // 解析Json对象并添加到容器中
    if (parser.failed) break;        // 如果解析失败，中断循环

    // 检查是否还有其他对象
    parser.consumeGarbage();
    if (parser.failed) break;  // 如果发现垃圾字符或有错误，中断循环
