...
bool RedisValueParser::consumeComment() {
  ...
  if (str[i] == '/') {
    i++;
    if (i == str.size()) return fail("在注释开始后意外结束输入", false);
    if (str[i] == '/') {  // 行内注释
      i++;
      // 前进直到下一行或输入结束
...
void RedisValueParser::consumeGarbage() { consumeWhitespace(); }

char RedisValueParser::getNextToken() {
  consumeGarbage();                         // 跳过空白字符和注释
  if (failed) return static_cast<char>(0);  // 如果解析失败，返回0
  if (i == str.size())
    return fail("意外到达输入的末尾",
                static_cast<char>(0));  // 到达输入末尾，标记错误并返回0
...
void RedisValueParser::encodeUTF8(long pt, std::string &out) {
  ...
  if (pt < 0x80) {
    ...
    out += static_cast<char>(((pt >> 6) & 0x3F) |
                             0x80);  // 3字节UTF-8编码的第二个字节
    ...
    out += static_cast<char>(((pt >> 12) & 0x3F) |
                             0x80);  // 4字节UTF-8编码的第二个字节
    out += static_cast<char>(((pt >> 6) & 0x3F) |
                             0x80);  // 4字节UTF-8编码的第三个字节
...
std::string RedisValueParser::parseString() {
  ...
  long last_escaped_codepoint =
      -1;  // 用于存储上一个转义的Unicode码点，初始化为-1
  while (true) {
    if (i == str.size()) return fail("在字符串中意外遇到输入结束", "");
    ...
    if (ch == '"') {
      encodeUTF8(
          ...
          out);  // 将上一个转义的Unicode码点编码为UTF-8并添加到输出字符串
    ...
    encodeUTF8(last_escaped_codepoint,
               out);  // 将上一个转义的Unicode码点编码为UTF-8并添加到输出字符串
...
RedisValue RedisValueParser::expect(const std::string &expected,
                                    RedisValue res) {
  ...
  if (str.compare(i, expected.length(), expected) == 0) {
    ...
    return fail("解析错误：期望 " + expected + "，但实际得到 " +
...
RedisValue RedisValueParser::parseRedisValue(int depth) {
  ...
  if (ch == '{') {                           // 如果是对象开始
    ...
    while (1) {
      if (ch != '"') return fail("在对象中期望 '\"'，得到 " + esc(ch));

      std::string key = parseString();  // 解析键
      if (failed) return RedisValue();
  ...
  if (ch == '[') {                 // 如果是数组开始
    ...
    while (1) {
      ...
      ch = getNextToken();
      if (ch == ']') break;
      if (ch != ',') return fail("在数组中期望 ','，得到 " + esc(ch));
